  ...
  <div class="content_wrap">
    ...
    <div class="match_form">
      ...
      if ($row['reply_date']) {
        ...
        <div class="answerBox">
          <div class="match_content match_box mt30">
            ...
            <textarea class="match_content_inner fwb fs16" readonly><?= $row['reply_content'] ?></textarea>
            <p class="li_title">사진</p>
            <div class="match_photos">
              <div class="match_photo">
                <?php
                $dir = "../uploads/inquiry/" . $row['u_id'] . '/' . $row['image_path'] . '/reply';
                if (is_dir($dir)) {
                  if ($dh = opendir($dir)) {
                    while (($file = readdir($dh)) !== false) {
                      if ($file != "." && $file != "..") { ?>
      ...
      <div class="modal_window modalAnswer">
        <div class="modal">
          <form action="userCounselCheck_reply.php?id=<?= $_GET['id'] ?>" method="post" enctype="multipart/form-data">
            <h3 class="modal_title">답변내용첨부</h3>
            <textarea class="modal_content w100" name="reply_content"><?= $row['reply_content'] ?></textarea>
            <div class="df">
              <h3 class="modal_title">이미지첨부</h3>
              <label for="answer_img">
                <img src="./images/icon_upload.png" alt="icon_upload">
              </label>
              <input type="file" id="answer_img" class="dn" name="upload[]" multiple>
